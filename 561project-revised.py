...
for i in df_num.columns:
    ...
    plt.title(i)
    plt.show()
    headers =[key for key in dict(df1.dtypes) if dict(df1.dtypes)[key] in ['float64']]
    print("The mean of variable '", str(headers[1]),"' is", np.mean(df_num[i]))
    print("The mean of standard deviation '", str(headers[1]),"' is", np.std(df_num[i]))
plt.close()
...
for i in item_nbr:
    ...
    def decisionTree_item_nbr(df, item_nbr):
        ...
        num_attr = ['tmax', 'tmin', 'tavg', 'dewpoint', 'wetbulb', 'heat', 'cool', 'preciptotal', 
                    'stnpressure','sealevel', 'resultspeed', 'resultdir', 'avgspeed', 'Weather', 'units']
        df = df[num_attr]
